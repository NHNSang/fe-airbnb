...
const CategoryList = () => {
    ...
    const settings = {
        dots: false,
        infinite: true,
        slidesToShow: isMobile ? 3 : isTablet ? 4 : isPcSD ? 8 : 10,
        slidesToScroll: 1,
        autoplay: true,
        autoplaySpeed: 1500,
        speed: 500,
        ...
        // <div className='flex flex-row justify-start items-center gap-8'>
            <Slider {...settings} >
                {categoryData.map((cate, index) => {
                    return (
                        <div key={index}
                            className='flex flex-col justify-center items-center text-neutral-500 dark:text-white hover:text-neutral-800 hover:scale-110  cursor-pointer transition duration-300'
                        >
                            <div>
                                {cate.src}
                            </div>
                            <p className='text-[10px] md:text-sm lg:text-sm'>{cate.title}</p>
                        </div>
                    )
                })}
            </Slider>
